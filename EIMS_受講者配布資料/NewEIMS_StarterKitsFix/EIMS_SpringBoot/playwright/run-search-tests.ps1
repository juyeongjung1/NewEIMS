...
<title>EIMS 検索機能 実装診断レポート</title><style>
...
</style></head><body><header><div><h1>EIMS 検索機能 実装診断レポート</h1><p>テストを開始する前に問題が見つかりました。</p></div></header><main><section class="card"><span class="badge">テストを開始できませんでした</span><h2>$EncodedTitle</h2><p>$EncodedMessage</p><div class="hint"><strong>まず確認してください</strong><ul><li>MySQLが起動しているか</li><li>Java 17とNode.jsがインストールされているか</li><li>Spring Bootのコンパイルエラーがないか</li><li>application.propertiesのDB設定が正しいか</li></ul></div><details open><summary>技術的な詳細を見る</summary><pre>$EncodedDetail</pre></details></section></main></body></html>
"@
...
try {
    ...
    if (-not (Test-Path -LiteralPath $MavenWrapper)) {
        throw 'mvnw.cmdが見つかりません。starter-kitのフォルダー構成を確認してください。'
    }
