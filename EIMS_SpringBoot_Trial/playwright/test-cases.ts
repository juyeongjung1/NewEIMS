...
const searchTestCasesWithoutFeature: Omit<EimsTestCase, 'feature'>[] = [
  ...
  {
    id: 'TC030', title: 'トップページの主要メニュー表示', level: 'common',
    description: 'トップページに社員一覧・社員検索・新規登録の主要な導線が表示されることを確認する。',
    input: 'URL「/」へアクセスする。',
    expected: '「社員一覧」「社員検索」「新規登録」のリンクが表示される。',
    note: 'T0001 TC001、10.1 URLマッピング: GET / -> index',
    hints: ['index.htmlに3つの主要メニューへのリンクがあるか確認してください。'],
  },
  {
    id: 'TC031', title: 'トップページから社員一覧へ遷移', level: 'common',
    description: 'トップページの社員一覧リンクから社員一覧画面へ遷移できることを確認する。',
    input: 'トップページで「社員一覧」をクリックする。',
    expected: '社員一覧画面 employee_list が表示される。',
    note: 'T0001 TC002、UC001 基本フロー、GET /employeeList',
    hints: ['index.htmlの社員一覧リンクとGET /employeeListを確認してください。'],
  },
  {
    id: 'TC032', title: '社員一覧の全件表示', level: 'common',
    description: '初期データの社員が社員一覧に漏れなく表示されることを確認する。',
    input: '社員一覧画面を表示する。',
    expected: '初期社員10001～10020がすべて一覧に表示される。',
    note: 'T0001 TC003、6.4 初期データ仕様',
    hints: ['findAll()の結果をemployeesとしてModelへ設定しているか確認してください。', 'employee_list.htmlのth:eachを確認してください。'],
  },
  {
    id: 'TC033', title: '社員一覧の列構成', level: 'standard',
    description: '社員一覧の表示項目が仕様通りであることを確認する。',
    input: '社員一覧画面の表ヘッダーを確認する。',
    expected: '表は「社員番号」「氏名+カナ」「性別」「部署名」の4列で構成される。',
    note: 'T0001 TC004、1.1.3 一覧画面（共通ルール）',
    hints: ['employee_list.htmlのtheadを確認してください。'],
  },
  {
    id: 'TC034', title: '社員一覧の氏名・カナ表示', level: 'common',
    description: '社員一覧で氏名とカナが指定された連結形式で表示されることを確認する。',
    input: '社員番号10001の行を確認する。',
    expected: '「長嶋 陽翔 (ナガシマ ヒナタ)」と半角スペース区切りで表示される。',
    note: 'T0001 TC005、1.1.3 共通の表示ルール',
    hints: ['氏と名、氏カナと名カナの間に半角スペースを入れてください。'],
  },
  {
    id: 'TC035', title: '社員一覧の性別表示', level: 'standard',
    description: 'DBの性別値が画面表示用の文言へ変換されることを確認する。',
    input: '社員一覧の性別列を確認する。',
    expected: '性別列に「男性」と「女性」が表示される。',
    note: 'T0001 TC006、1.1.3 一覧画面（共通ルール）',
    hints: ['gender=1を男性、gender=2を女性として表示してください。'],
  },
  {
    id: 'TC036', title: '社員一覧の部署名表示', level: 'standard',
    description: '社員一覧に部署コードではなく部署名が表示されることを確認する。',
    input: '社員一覧の部署名列を確認する。',
    expected: '6部署の名称が部署名として表示される。',
    note: 'T0001 TC007、標準実装',
    hints: ['EmployeeからDepartmentのdeptNameを表示しているか確認してください。'],
  },
  {
    id: 'TC037', title: '社員一覧から詳細画面へ遷移', level: 'common',
    description: '社員一覧の氏名リンクから対象社員の詳細画面へ遷移できることを確認する。',
    input: '社員番号10001の氏名リンクをクリックする。',
    expected: '社員番号10001の社員詳細画面が表示される。',
    note: 'T0001 TC008、UC001 alt3-1、GET /detail/{empNo}',
    hints: ['氏名を/detail/{empNo}へのリンクにしてください。'],
  },
  {
    id: 'TC038', title: '社員詳細画面の表示項目', level: 'standard',
    description: '社員詳細画面に仕様通りの社員情報が表示されることを確認する。',
    input: '社員番号10001の詳細画面を表示する。',
    expected: '社員番号、氏、名、氏（カナ）、名（カナ）、性別、所属部署が表示される。',
    note: 'T0001 TC009、1.1.3 社員詳細画面',
    hints: ['employee_detail.htmlの7つの表示項目を確認してください。'],
  },
  {
    id: 'TC039', title: '詳細画面から変更画面へ遷移', level: 'common',
    description: '社員詳細画面から選択社員の変更画面へ進めることを確認する。',
    input: '社員番号10001の詳細画面で「情報を変更する」をクリックする。',
    expected: '社員番号10001の変更画面 change が表示される。',
    note: 'T0001 TC010、UC004の起点',
    hints: ['詳細画面から/changeInput/{empNo}へ遷移できるか確認してください。'],
  },
  {
    id: 'TC040', title: '詳細画面から削除確認へ遷移', level: 'common',
    description: '社員詳細画面から選択社員の削除確認画面へ進めることを確認する。',
    input: '社員番号10001の詳細画面で「削除する」をクリックする。',
    expected: '社員番号10001の削除確認画面 delete_confirm が表示される。',
    note: 'T0001 TC011、UC005の起点',
    hints: ['詳細画面から/deleteConfirm/{empNo}へ遷移できるか確認してください。'],
  },
  {
    id: 'TC041', title: '社員一覧からトップページへ戻る', level: 'common',
    description: '社員一覧画面からトップページへ戻れることを確認する。',
    input: '社員一覧画面で「メニューに戻る」をクリックする。',
    expected: 'トップページが正常に表示される。',
    note: 'T0001 TC012、UC001 alt3-2',
    hints: ['employee_list.htmlにトップページへの導線を配置してください。'],
  },
  {
    id: 'TC042', title: '社員一覧から検索画面へ遷移', level: 'common',
    description: '社員一覧画面から検索画面へ遷移できることを確認する。',
    input: '社員一覧画面で「検索画面へ」をクリックする。',
    expected: '検索画面 search が表示される。',
    note: 'T0001 TC013、UC001 alt3-3',
    hints: ['employee_list.htmlに/searchへの導線を配置してください。'],
  },
];
