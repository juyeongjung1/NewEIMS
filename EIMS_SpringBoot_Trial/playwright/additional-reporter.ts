...
import { additionalTestCases, type AdditionalGroup, type AdditionalTestCase } from './additional-test-cases';

const testCaseMap = new Map(additionalTestCases.map((testCase) => [testCase.id, testCase]));
const groupLabels: Record<AdditionalGroup, string> = {
  login: 'ログイン／ログアウト', design: 'デザイン改良', permission: '権限・詳細画面', retiree: '退職者管理',
};
const selectedTestCases = additionalTestCases;
const assessmentLabel = '追加機能・総合診断';

type CaseResult = {
  definition: AdditionalTestCase;
...
function levelLabel(level: AdditionalTestCase['level']): string {
  if (level === 'common') return '基本仕様';
...
class EimsReporter implements Reporter {
  ...
  private outputFile = path.resolve(process.cwd(), 'results', 'eims-additional-report.html');
  ...
  onTestBegin(test: TestCase): void {
    ...
    const match = test.title.match(/\[additional:(AT\d{3})\]/);
    const definition = match ? testCaseMap.get(match[1]) : undefined;
    if (!definition) return;

    this.sendProgress({
      percent: 30 + Math.round((this.completedCount / selectedTestCases.length) * 65),
      phase: groupLabels[definition.group],
  ...
  async onEnd(_result: FullResult): Promise<{ status: FullResult['status'] }> {
    ...
    const allResults = selectedTestCases.map((definition) => this.results.get(definition.id) ?? ({
      definition,
  ...
  private buildHtml(results: CaseResult[]): string {
    ...
    const overall = simplifiedComplete && standardComplete
      ? { label: '一般仕様達成', className: 'pass', description: `${subject}は依存関係を含む一般仕様まで実装できています。` }
      : simplifiedComplete
        ? { label: '基本仕様達成', className: 'warning', description: `${subject}の基本仕様は達成しています。黄色の項目へ進むと一般仕様を目指せます。` }
        : { label: '要確認', className: 'fail', description: `${subject}の基本仕様に問題があります。赤い項目から確認してください。` };
    ...
    const groupButtons = '<button data-filter="login">ログイン</button><button data-filter="design">デザイン</button><button data-filter="permission">権限・詳細</button><button data-filter="retiree">退職者管理</button>';
    ...
    const cards = results.map((item) => {
      ...
      const screenshot = item.screenshot
        ? `<div class="screenshot">
            ...
            <a class="shot-frame" href="${item.screenshot}" target="_blank">
              <strong class="shot-label">確認${escapeHtml(item.definition.id.slice(2))}・${escapeHtml(item.definition.title)}</strong>
      ...
      return `
        <article class="case ${status.className}" data-status="${status.className}" data-level="${item.definition.level}" data-group="${item.definition.group}">
          <details${detailOpen}>
            <summary>
              <span class="status-icon">${status.icon}</span>
              <span class="case-title"><small>${groupLabels[item.definition.group]}・確認${escapeHtml(item.definition.id.slice(2))}・${levelLabel(item.definition.level)}</small>${escapeHtml(item.definition.title)}</span>
...
<head>
  ...
  <title>EIMS 追加機能 総合診断レポート</title>
  <style>
...
<body>
  <header><div class="inner"><h1>EIMS 追加機能 総合診断レポート</h1><p>${assessmentLabel}</p><div class="accuracy-note">この診断レポートは100%正確とは限りません。実際の動作やテスト結果と差異がある場合は、講師へ確認してください。</div></div></header>
  <main>
    <section class="hero">
      <div class="panel overall ${overall.className}"><span class="badge">${overall.label}</span><p>${overall.description}</p></div>
      <div class="panel scores">
        ${this.scoreCard('基本仕様', simplifiedPassed, common.length)}
    ...
    <section class="panel toolbar">
      ...
      <button data-filter="common">基本仕様</button>
    ...
    <p class="legend">基本仕様を達成したあと、黄色の項目へ進むと一般仕様を目指せます。参考確認は総合判定に影響しません。</p>
  </main>
  <script>
    ...
    buttons.forEach((button) => button.addEventListener('click', () => {
      ...
      cases.forEach((item) => {
        ...
        item.hidden = !(filter === 'all' || item.dataset.level === filter || item.dataset.group === filter || (filter === 'problem' && problem));
      });
    }));
  </script>
</body>
