...
##### 【社員名検索の入力例】
受講生は、以下の入力例に基づき検索ロジックを実装してください。
- **例 1**: 「田」が入力された場合、**氏（lname）** または **名（fname）** の列で「田」を含む社員を検索対象とする。
...
- **検索結果一覧画面**:
    - 以下の 5 項目をテーブル形式で複数行表示する。
        1. **社員番号**
        2. **氏名**（氏と名の間は半角スペースで連結）
        3. **カナ**（氏カナと名カナの間は半角スペースで連結）
        4. **性別**（DB値が 1 なら「男性」、2 なら「女性」と置換して表示）
        5. **部署名**
