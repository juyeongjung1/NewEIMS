...
本セクションでは、各画面の構成要素（テーブル形式）とワイヤーフレーム（画面イメージ）を定義します。開発時はこの設計図をベースにHTML/CSSを構築してください。
...
| 項目名 | タイプ | 説明 |
|---|---|---|
| アプリタイトル | Header | `EIMS Web` (全画面共通) |
| 社員情報を検索する | Link | `検索条件入力画面 (UI2)` へ遷移 |
| 社員情報を登録する | Link | `社員情報登録画面 (UI4)` へ遷移 |
...
| 画面タイトル | Text | `社員情報の検索` |
| 社員番号 | Input | 半角数字 |
| 氏・名 | Input | 文字列（氏・名のどちらかを含む） |
| 部署名 | Select | 部署テーブルから取得したプルダウン |
| 検索 | Button | `検索結果一覧画面 (UI3)` または `詳細画面 (UI_D)` へ遷移 |
| トップページへ戻る | Link | `トップページ (UI1)` へ遷移 |
...
#### UI3: 検索結果一覧画面
検索条件（氏・名・部署）に合致する社員を一覧で表示する画面。

**【画面要素】**
| 項目名 | タイプ | 説明 |
|---|---|---|
| 画面タイトル | Text | `検索結果一覧` |
| 検索結果テーブル | Table | 社員番号、氏名+カナ(リンク)、性別、部署名の4列 |
| 氏名(カナ) | Link | `社員詳細画面 (UI_D)` へ遷移 |
| エラーメッセージ | Text | 0件の場合「該当する社員は存在しませんでした」と表示 |
| 検索画面へ戻る | Link | `検索条件入力画面 (UI2)` へ遷移 |
| トップページへ戻る | Link | `トップページ (UI1)` へ遷移 |
...
┃ │社員番号│氏名(カナ)            │性別  │部署名│ ┃
┃ ├──────┼──────────────────────┼──────┼──────┤ ┃
┃ │1001    │[山田 太郎(ヤマダ タロウ)]│男性  │営業部│ ┃
┃ │1002    │[佐藤 花子(サトウ ハナコ)]│女性  │人事部│ ┃
┃ └──────┴──────────────────────┴──────┴──────┘ ┃
┃                                                     ┃
┃  ＜検索画面へ戻る     ＜トップページへ戻る          ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
...
#### UI_D: 社員詳細画面
社員のすべての情報を表示し、変更・削除の起点となる画面。

**【画面要素】**
| 項目名 | タイプ | 説明 |
|---|---|---|
| 画面タイトル | Text | `社員詳細情報` |
| 社員情報表示 | Text | 社員番号、氏名(カナ)、性別、部署名 |
| 変更 | Button | `変更画面 (UI9)` へ遷移 |
| 削除 | Button | `削除確認画面 (UI7)` へ遷移 |
| 検索画面へ戻る | Link | `検索条件入力画面 (UI2)` へ遷移 |
| トップページへ戻る | Link | `トップページ (UI1)` へ遷移 |
