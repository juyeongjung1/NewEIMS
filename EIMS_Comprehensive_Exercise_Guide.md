...
|---|---|
| **アクター** | 人事部管理者 |
| **概要** | 社員番号・氏名・部署を条件に社員情報を検索し、結果を一覧または詳細で表示する。 |
| **事前条件** | トップページが表示されている。 |
| **事後条件** | 検索結果が画面に表示される。 |

**基本フロー:**
1. 管理者がトップページから検索画面を開く。
2. 検索条件（社員番号 / 氏名 / 部署）を入力する。
3. 検索ボタンを押下する。
4. システムが条件に一致する社員情報を検索し、結果を表示する。

**代替フロー:**
- **2a.** 社員番号で検索した場合、検索結果一覧を経由せず直接「社員詳細画面」を表示する。
- **4a.** 検索結果が 0 件の場合、「該当する社員は存在しませんでした」とメッセージを表示する。
- **4b.** 入力値が空の場合、検索画面に留まる。
...
|---|---|
| **アクター** | 人事部管理者 |
| **概要** | 新しい社員の情報を入力し、データベースに登録する。 |
| **事前条件** | トップページが表示されている。 |
| **事後条件** | 新しい社員レコードがデータベースに保存される。 |

**基本フロー:**
1. 管理者がトップページから登録画面を開く。
2. 社員情報（氏名、カナ、パスワード、性別、部署）を入力する。
3. 登録ボタンを押下する。
4. システムが入力値を検証し、登録確認画面を表示する。
5. 管理者が登録確定ボタンを押下する。
6. システムがデータベースに保存し、登録完了画面を表示する。

**代替フロー:**
- **4a.** バリデーションエラーの場合、登録画面に戻りエラーメッセージを表示する。入力値は保持される。
- **5a.** 修正するボタンを押下した場合、入力値を保持した状態で登録画面に戻る。
...
|---|---|
| **アクター** | 人事部管理者 |
| **概要** | 指定された社員の情報をデータベースから削除する。 |
| **事前条件** | U001（検索）により対象社員が特定され、社員詳細画面が表示されている。 |
| **事後条件** | 対象社員のレコードがデータベースから物理削除される。 |

**基本フロー:**
1. 管理者が社員詳細画面の削除ボタンを押下する。
2. システムが削除確認画面を表示する。
3. 管理者が削除確定ボタンを押下する。
4. システムがデータベースからレコードを削除し、削除完了画面を表示する。

**代替フロー:**
- **2a.** 詳細画面に戻るボタンを押下した場合、削除を中止し詳細画面に戻る。
...
|---|---|
| **アクター** | 人事部管理者 |
| **概要** | 既存の社員情報を変更し、データベースを更新する。 |
| **事前条件** | U001（検索）により対象社員が特定され、社員詳細画面が表示されている。 |
| **事後条件** | 対象社員のレコードがデータベース上で更新される。 |

**基本フロー:**
1. 管理者が社員詳細画面の変更ボタンを押下する。
2. システムが現在の登録データをセットした変更画面を表示する。
3. 管理者が変更内容を入力する。
4. 変更ボタンを押下する。
5. システムが入力値を検証し、変更確認画面を表示する。
6. 管理者が変更確定ボタンを押下する。
7. システムがデータベースを更新し、変更完了画面を表示する。

**代替フロー:**
- **5a.** バリデーションエラーの場合、変更画面に戻りエラーメッセージを表示する。入力値は保持される。
- **6a.** 修正するボタンを押下した場合、入力値を保持した状態で変更画面に戻る。
