...
</style></head><body><header><div><h1>EIMS 共通機能 実装診断レポート</h1><p>検索・登録・更新・削除</p><p>この診断レポートは100%正確とは限りません。実際の動作やテスト結果と差異がある場合は、講師へ確認してください。</p></div></header><main><section class="card"><span class="badge">テストを開始できませんでした</span><h2>$EncodedTitle</h2><p>$EncodedMessage</p><div class="hint"><strong>まず確認してください</strong><ul><li>MySQLが起動しているか</li><li>Java 17とNode.jsがインストールされているか</li><li>Spring Bootのコンパイルエラーがないか</li><li>application.propertiesのDB設定が正しいか</li></ul></div><details open><summary>技術的な詳細を見る</summary><pre>$EncodedDetail</pre></details></section></main></body></html>
"@
