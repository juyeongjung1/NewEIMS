...
const reportPath = join(root, 'results', 'eims-additional-report.html');
const settings = { label: '追加機能・総合診断', total: 47 };
...
function startDiagnostics() {
  ...
  running = true;
  publish({ percent: 1, phase: '開始', message: '追加機能の総合診断を開始しました。', status: 'running', total: settings.total });
  const child = spawn('powershell.exe', [
    '-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass',
    '-File', join(root, 'run-additional-tests.ps1'), '-NoOpenReport',
  ...
  child.on('exit', (code) => {
    ...
    const status = commonFailures > 0 || (code !== 0 && standardFailures === 0)
      ? 'failed'
      : standardFailures > 0 ? 'simplified' : 'completed';
    publish({
      percent: 100,
      phase: status === 'completed' ? '完了' : status === 'simplified' ? '基本仕様達成' : '要確認',
      message: status === 'completed'
        ? '追加機能の一般仕様まで、すべての確認が完了しました。'
        : status === 'simplified'
          ? '追加機能の基本仕様は達成できましたが、一般仕様には未達成の項目があります。'
...
const server = createServer((request, response) => {
  ...
  if (request.method === 'GET' && url.pathname === '/') {
    ...
    response.end(readFileSync(dashboardPath)); return;
  }
  if (request.method === 'GET' && url.pathname === '/settings') {
    response.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' });
    response.end(JSON.stringify(settings)); return;
  }
  ...
  if (request.method === 'POST' && url.pathname === '/start') {
    startDiagnostics(); response.writeHead(202, { 'Content-Type': 'application/json' }); response.end('{"started":true}'); return;
  }
  if (request.method === 'GET' && url.pathname === '/report' && existsSync(reportPath)) {
    response.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
    createReadStream(reportPath).pipe(response); return;
  }
